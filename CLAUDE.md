...
│   │   ├── analysis/
│   │   │   ├── budget-optimization-analysis.types.ts # ConfidenceLevel/ExecutionRisk types; InefficientCampaignSignal (extends ShareEfficiency + campaign/channel/roi/maxReducibleBudget/reason); BudgetScalingCandidate (extends ShareEfficiency + campaign/channel/roi/maxAdditionalBudget/expectedRoiRetention/reason); TransferCandidate (fromCampaign/toCampaign/minShift/maxShift/expectedRoiRetention/reason); BudgetOptimizerAnalysis (portfolio/campaigns/channels/derivedSignals)
│   │   │   ├── budget-optimization-analysis.ts # getInefficientCampaigns / getBudgetScalingCandidates / getTransferCandidates; computeChannelStatus / toCampaignSummary / toChannelSummary; computeBudgetOptimizerAnalysis(campaigns, channels, kpis, scope) → BudgetOptimizerAnalysis — full assembly including derivedSignals
│   │   │   ├── executive-summary-analysis.types.ts # SummaryMetricStatus/HealthLabel/InsightType/ActionUrgency/ConcentrationLevel enums; ChannelSummary (extends CampaignMetrics + PerformanceMetrics + ShareEfficiency + status); CampaignSummary (extends CampaignPerformance + ShareEfficiency); PortfolioSummary (extends PortfolioKPIs + campaignCount/channelCount); SummaryAnalysis (portfolio: PortfolioSummary, channels, topCampaigns, bottomCampaigns, derivedSignals, businessContext?); signal types (InefficientChannelSignal, ScalingCandidateSignal, ConcentrationFlagSignal, CorrelationSignal)
│   │   │   └── executive-summary-analysis.ts # getTopCampaigns / getBottomCampaigns (excludes top names, min 1% budgetShare) / getInefficientChannels (GAP_THRESHOLD=0.05, roi < portfolioRoi) / getScalingCandidates (campaign + channel candidates, top 5 by ROI) / getConcentrationFlag / getCorrelations (stub) / getExecutiveSummaryDerivedInputs — computes top first, passes excluded set to bottom; computeChannelStatus / toCampaignSummary / toChannelSummary / computeSummaryAnalysis(campaigns, channels, kpis, scope) → SummaryAnalysis — full assembly including derived signals
│   │   └── data/
...
│   │   └── aiAnalysisStore.ts  # Pinia store — shared AI analysis logic for both tabs; per-tab internal state (plain object): firstAnalyzeCompleted, controller, debounceTimer, cache (Map<string, CacheEntry> — response/timestamp/cooldownUntil), dataCache (Map<string, BudgetOptimizerAnalysis|SummaryAnalysis>), lastVisibleCacheKey; per-tab reactive display state (reactive()): optimizer { status, response: BudgetOptimizerResponse|null, error, errorFallback } + summary { same shape, ExecutiveSummaryResponse|null }; shared: activeTab, tokenLimitReached, analysisActivated; debounced label-change auto-calls (300ms), response caching (provider::sorted labels), data caching (labels), request cancellation (AbortController), 5s cooldown, silent model fallback on token-limit, stamps response.model + response.timestamp on success, model change watcher, setActiveTab cancels in-flight on previous tab, panel open/close + tab switch evaluation, clearStateForNewCSV, clearStateForDisconnect
│   ├── router/
...
│   │   │   │       │   ├── BudgetOptimizationAnalysis.vue        # Budget Optimizer tab — thin orchestrator; wraps AnalysisState; renders Overview + Recommendations only; reads campaignStore + aiAnalysisStore; no scoped styles
...
│   │   │   ├── types/
│   │   │   │   └── index.ts            # AiProviderType, AiErrorCode (unified 10-code union), AiConnectionError, AiAnalysisError, AiAnalysisTab, ConfidenceLevel + shared building blocks (CampainSummaryTotals, AllocationShare, FunnelMetrics, PortfolioCount, Correlation) + prompt types (PromptList, PromptInstructions, PromptInstructionStep, PromptScopeConfig, BusinessContext, BudgetOptimizerContextInput) + legacy data type (BudgetOptimizerData — used by old prompt file) + response types (BudgetOptimizerResponse = BudgetOptimizerOutput & {model?,timestamp?}; ExecutiveSummaryResponse = ExecutiveSummaryOutput & {model?,timestamp?})
│   │   │   ├── prompts/
...
│   │   │   │   ├── executive-summary-prompt2.ts # generateExecutiveSummaryPrompt(input: SummaryAnalysis, filteredChannels: boolean) — assembles executive-summary AI prompt; camelCase schema; derivedSignals-first prioritization; strict noise-control and correlation rules
│   │   │   │   ├── budget-optimization-prompt2.ts # generateBudgetOptimizationPrompt(input: BudgetOptimizerAnalysis, filteredChannels: boolean, businessContext?) — assembles budget-optimizer AI prompt; local getScopeBlock + getBusinessContextBlock; derivedSignals-first; camelCase OUTPUT_SCHEMA (summary + recommendations[])
...
│   │   │   ├── utils/
│   │   │   │   ├── buildBudgetOptimizerData.ts  # Legacy — transforms Campaign[] into BudgetOptimizerData; no longer used by aiAnalysisStore (replaced by computeBudgetOptimizerAnalysis)
