...
│   │   │   │   ├── AiOptimizerPanel.vue    # Budget Optimizer tab — .btn-primary Analyze/Re-Analyze; idle/loading (Spinner xxl)/done/error states; renders full BudgetOptimizerResponse using .badge + BadgeVariant class pairs (no Badge component); .card-secondary for all cards; .badge-background.badge-text.opportunity for quick win details; cached indicator with timestamp, error fallback message, token-limit notice; wired to aiAnalysisStore
│   │   │   │   └── AiSummaryPanel.vue      # Executive Summary tab — .btn-primary Summarize/Re-Summarize; idle/loading (Spinner xxl)/done/error states; renders full ExecutiveSummaryResponse using .badge + BadgeVariant class pairs; .roi-text + roiClass() from common/utils/roi for ROI values; .badge-background.badge-text for insight metric highlights; cached indicator with timestamp, error fallback message, token-limit notice; wired to aiAnalysisStore
│   │   │   ├── ai-analysis/
