...
│   │   │   │   ├── AiOptimizerPanel.vue    # Budget Optimizer tab — wraps AiAnalysisState; renders full BudgetOptimizerResponse sections (AiAnalysisSummary, recommendations, top/underperformers, quick wins, AiAnalysisCorrelations, risks) in default slot; imports badge variants and formatters from utils; reads campaignStore for campaign counts; wired to aiAnalysisStore
│   │   │   │   └── AiSummaryPanel.vue      # Executive Summary tab — wraps AiAnalysisState; renders full ExecutiveSummaryResponse sections (AiAnalysisSummary with health badge in #badge slot + bottom line in default slot, priority actions, key metrics, insights, channel summary, AiAnalysisCorrelations) in default slot; imports badge variants and formatters from utils; classROI() local (delegates to roiClass); reads campaignStore for campaign counts; wired to aiAnalysisStore
│   │   │   ├── ai-analysis/
