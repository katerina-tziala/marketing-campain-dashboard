...
│   │   ├── Button.vue          # Generic button wrapper — props: disabled?, type? (button/submit/reset); class pass-through for scoped modifier classes: .btn.primary (solid bg-primary-dark, hover bg-primary-light/text-primary-ink, focus-visible ring), .btn.outline (transparent bg, border-current matches text color, hover bg-background, focus-visible ring), .btn.text-only (no border/bg, hover bg-background, focus-visible ring; .no-ring modifier suppresses ring), .btn.destructive (neutral text, hover bg-background text-danger, focus-visible danger ring), .btn.icon-only (8×8 square, p-0), .btn.small (xs text, tight padding); no ::after overlay — hover handled via direct bg/color on the button element; disabled: cursor-not-allowed opacity-50 (base)
