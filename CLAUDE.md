...
│   │   │   │   ├── model-selection-prompt.ts   # generateModelSelectionPrompt — selects top 5 models (legacy)
│   │   │   │   ├── model-evaluation-prompt.ts  # generateModelEvaluationPrompt — evaluates and ranks up to 15 models by strength_score (active)
...
│   │   │   │   └── rankModels.ts                # rankModels(parsed, optimalModelId, fallback) — sorts AI-evaluated models by strength_score desc, inits limitReached, updates optimal model properties from AI response
