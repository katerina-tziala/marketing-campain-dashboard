...
│   │   ├── Button.vue          # Generic button wrapper — props: disabled?, type? (button/submit/reset); class pass-through for scoped modifier classes: .btn.primary (solid bg-primary-dark, hover bg-primary-light/text-primary-ink, focus-visible ring), .btn.outline (transparent bg, border-current matches text color, hover bg-background, focus-visible ring), .btn.text-only (no border/bg, hover bg-primary-light/15, focus-visible ring-primary-lighter), .btn.ghost (neutral text-typography-soft base, hover bg-typography/10 text-typography, focus ring-typography-soft; for close/dismiss chrome buttons), .btn.destructive (neutral text, hover bg-background text-danger, focus-visible danger ring), .btn.icon-only (8×8 square, p-0), .btn.small (xs text, tight padding); no ::after overlay — hover handled via direct bg/color on the button element; disabled: cursor-not-allowed opacity-50 (base)
...
│   │   ├── Chip.vue            # Chip button — props: active?, readonly?; default slot for projected content; active drives border-primary/bg-primary/text-on-primary + aria-pressed; inactive drives muted surface + hover/focus-visible primary-light; readonly adds cursor-default pointer-events-none; Tailwind only, no style block
...
│   │   │   ├── DropdownPanel.vue # Dropdown content shell — props: ariaLabel?; role="dialog"; visual container (bg-surface-elevated border rounded-xl shadow-lg); no scroll by default — callers add overflow on inner wrappers as needed; default slot
...
│   │   │           ├── index.ts            # Barrel — exports ChannelFilters, ChannelFiltersDropdown
│   │   │           ├── ChannelFilters.vue  # Two-state filter strip — State A (all fit, ≤4.5rem): All chip first (interactive, clears filter) + all channel chips interactive; State B (overflow): SlidersIcon trigger + clipped single-row strip; State B no selection: read-only All chip only; State B with selection: selected chips sorted by name, interactive (click deselects); hidden measureRef container (absolute left:0 right:0, always All + all chips) drives hasOverflow via scrollHeight — decouples overflow detection from display content; measure split into measureOverflow() (measureRef.scrollHeight vs 4.5rem) and measureHidden() (offsetTop on chipsRef chips); notification count badge on filter button shows count of hidden selected chips; ResizeObserver on measureRef; window resize closes dropdown; body scroll locked when dropdown open; dropdown Teleport (z-50, boundary-aware fixed positioning); backdrop (position:fixed inset-0 z-49, Teleport to body); max-height via --channel-filter-max-height CSS var (default 4.5rem)
│   │   │           └── ChannelFiltersDropdown.vue  # Floating dropdown panel — no self-positioning (parent Teleport wrapper owns fixed coords); uses DropdownPanel as outer shell; inner scroll wrapper (max-h-[300px] overflow-y-auto) wraps sticky header + chips; all channels as toggle chips + sticky "Channels" header + "Clear all" (shown when selection exists); emits toggle/clear/close; props-only, no store reads
