...
│   │   │   ├── CheckIcon.vue       # Plain checkmark (polyline) — used in DuplicationGroupHeader "Resolved" badge
│   │   │   ├── CircleCheckIcon.vue # Solid circle with checkmark — pending/awaiting state; used in DuplicationGroupHeader "Select one" badge
│   │   │   ├── ClockIcon.vue       # Clock face with hour/minute hand
│   │   │   ├── CloseIcon.vue
│   │   │   ├── DownloadIcon.vue
│   │   │   ├── EyeIcon.vue         # Show password icon
│   │   │   ├── EyeOffIcon.vue      # Hide password icon
│   │   │   ├── FileTextIcon.vue
│   │   │   ├── FunnelIcon.vue      # Filter/funnel icon — filled polygon (wide top, narrowing to small bottom stem); used as filter trigger in ChannelFilters
...
│   │   │   ├── RadioItem.vue       # Single custom radio input — props: name, value (string|number), checked, ariaLabel?, error?, disabled?; emits change:[value]; hidden native input (aria-label on input) + custom circle indicator (::before inner dot); primary color palette for all states (unchecked/hover/checked/checked+hover/focus-visible); error state uses danger tokens; disabled opacity-40; scoped flat styles
...
│   │   ├── modal/              # Modal component module
│   │   │   ├── Modal.vue       # Generic modal shell — Teleport to body; backdrop (click-to-close via @click.self, aria-modal/role="dialog"/aria-label); header (title prop + close button), single default slot; Escape to close; scoped .modal-backdrop + .modal + .modal-title styles
│   │   │   ├── ModalBody.vue   # Modal body wrapper — default slot; scoped .modal-body (flex-col gap-3 p-6 w-fit max-w-2xl)
│   │   │   ├── ModalFooter.vue # Modal footer wrapper — default slot; scoped .modal-footer (flex-col → xs:flex-row-reverse, border-t, py-5 px-6, gap-4)
│   │   │   └── index.ts        # Barrel — exports Modal, ModalBody, ModalFooter
...
│   │   ├── Button.vue          # Generic button wrapper — props: disabled?, type? (button/submit/reset); exposes getRootEl() → HTMLButtonElement for dropdown anchoring; class pass-through for scoped modifier classes: .btn.primary (solid bg-primary-dark, hover bg-primary-light/text-primary-ink, focus-visible ring), .btn.outline (transparent bg, border-current, hover bg-primary-light/15, focus-visible ring), .btn.text-only (no border/bg, hover bg-primary-light/15, focus-visible ring-primary-lighter), .btn.ghost (neutral text-typography-soft base, hover bg-typography/10 text-typography, focus ring-typography-soft; for close/dismiss chrome buttons), .btn.destructive (neutral text, hover bg-danger-darker/10 text-danger, focus-visible danger ring), .btn.info-outline (surface bg + subtle text + single border; hover/focus → border-info/65 text-info-light; chip-aligned filter trigger style), .btn.icon-only (8×8 square, p-0), .btn.small (xs text, tight padding), .btn.paddingless (p-0); no ::after overlay — hover handled via direct bg/color on the button element; disabled: cursor-not-allowed opacity-50 (base)
│   │   ├── card/               # Card component module
│   │   │   ├── Card.vue        # Generic card wrapper — no props; renders <article class="card">; modifier classes (e.g. secondary) applied at call site via Vue class fallthrough; non-scoped global styles: .card (default) + .card.secondary (compound selector); .card-secondary removed — secondary variant requires both classes on the element
...
│   │   ├── Badge.vue           # Generic badge — two-layer structure: outer `.badge` (shape + bg-primary-ink) + inner `.badge-body` (border/padding/color); variant applied as class on outer element; variants: success/warning/danger/info/opportunity; modifiers: dimmed (muted palette), rounded-rectangle, rounded-rectangle-sm, text-only, small (tighter py-0.5 px-1 padding), bold (font-bold inner text); no variant prop — class-based only
...
│   │   ├── SheetHeader.vue     # Layout-only panel/modal header — three optional named slots: #icon (leading icon, text-primary-lighter) + #header (title text) share a grow flex row; #action (trailing button) is outside the grow group; gradient (from-accent-light→to-secondary-light) applied via :slotted(h1–h6) with bg-clip-text text-transparent w-fit directly on projected headings; border-b separator; no props, no store reads
...
│   │   ├── Chip.vue            # Chip button — props: active?, readonly?, count?; default slot for label text; count renders an inline badge (bg-on-primary/10) when provided; active state driven by [aria-pressed="true"] → bg-background border-info/65, inner .chip-content bg-info/15 text-info-light; hover/focus-visible (non-readonly) → border-info/65 text-info-light; focus-visible → ring-2 ring-info-dark; readonly → cursor-default pointer-events-none; scoped SCSS block
│   │   ├── Disclosure.vue      # ARIA disclosure pattern — no props; manages isOpen internally; generates unique contentId; #trigger scoped slot exposes { open, toggle, contentId } — caller owns button element, label, and aria-controls binding; default slot is the disclosed content; Transition uses :css="false" + JS hooks (@enter/@leave) to animate height from 0→scrollHeight via transitionend + done callback — no max-h hack
│   │   ├── dropdown/           # Dropdown module
│   │   │   ├── Dropdown.vue    # Generic floating dropdown shell — props: open (v-model:open), anchor (HTMLElement|null), minWidth?, maxHeight?, gap?, edgeMargin?; default slot for content; teleports backdrop (aria-hidden, z-49) + floating panel (z-50) to body; boundary-aware fixed positioning from anchor rect (flips above when no room below); locks body scroll when open; on open focuses first focusable element in panel; on close returns focus to anchor; closes on backdrop click, Escape key, or window resize; emits update:open
│   │   │   ├── DropdownPanel.vue # Dropdown content shell — props: ariaLabel?; role="dialog"; visual container (bg-surface-raised border rounded-md shadow-lg overflow-hidden pb-2); no scroll — callers add their own overflow wrapper inside; default slot
│   │   │   └── index.ts        # Barrel — exports Dropdown, DropdownPanel
│   │   └── index.ts            # Barrel export for the full ui library — re-exports charts/*, icons/*, toast/*, types/*, meta/*, modal/* (Modal, ModalBody, ModalFooter); wildcard re-exports card/* (Card, CardHeader); exports Tabs, Spinner, DataTableHeader, FileDropzone, PasswordInput, RadioToggle, RadioItem, Button, ButtonVariant, ButtonSize, Badge, Chip, SectionHeaderLayout, Notification, SheetHeader, Disclosure; re-exports dropdown/* (Dropdown, DropdownPanel)
│   ├── shell/
│   │   ├── AppShell.vue            # Top-level layout wrapper — flat @apply styles (shell-header/shell-title/shell-body/shell-main); flex col always: full-width header + shell-body row (main + drawer) below; header never compressed by AI drawer; provides openUploadModal and openAiPanel via provide(); uses useAiConnectionStore.aiPanelOpen for panel state; wires panel open/close to aiAnalysis.store; header "Upload CSV" uses Button.outline and routes through ReplaceDataModal when data exists; gradient title (indigo→pink)
...
│   │   │   │   │   ├── AiConnectionForm.vue        # Provider selection via RadioToggle + API key via PasswordInput + Connect button + Disclosure (instructions, after Connect); providerHelp computed from PROVIDER_HELP; clears connectionError + apiKey on provider change; imports ERROR_MESSAGES, ERROR_HINTS from utils/error-handling; flat scoped styles (no BEM)
│   │   │   │   │   ├── AiConnectionInstructions.vue # Instructions card — props: instructions ({ title, steps, note? }); uses <Card class="secondary"> with title h5, ordered steps list, optional note, and static privacy warning; scoped .help-steps + .help-note styles
...
│   │   │       ├── DashboardHeader.vue # Dashboard header — reads campaign.store (title, campaign/channel counts via selectedChannelCount computed) + useAiConnectionStore (isConnected, aiPanelOpen); emits aiClick (camelCase); multi-root (dashboard-title-row + MetaRow); MetaRow (bullet variant) shows portfolio title / selected-of-total channels / selected-of-total campaigns using MetaItem; AI button (Button.primary) hidden with v-if when panel open; connected-dot shown when AI connected + panel closed; dot has scoped dot-pop @keyframes (cubic-bezier spring, scale 0→1); layout wrapper provided by DashboardView
│   │   │       ├── DashboardKpis.vue   # KPI cards section — props: kpis (PortfolioKPIs), portfolioKpis? (PortfolioKPIs|null); local formatShare helper (1-decimal "X% of total"); Budget/Revenue/Conversions show "X% of total" when portfolioKpis present; Revenue adds ROI via RoiIndicator; Conversions adds CVR via RoiIndicator; CTR uses KpiBenchmarkDelta (unit="pp"); CPA uses KpiBenchmarkDelta (unit="pct", lowerIsBetter, showAbsoluteCurrency); .kpi-grid uses @container breakpoints (360px → 2 cols, 640px → 3 cols, 1024px → 5 cols)
...
│   │   │       ├── RoiBudgetScatter.vue # ROI vs Budget scaling opportunities scatter — props: campaigns (CampaignPerformance[]), isFiltered? (boolean); Bubble from vue-chartjs; 4 quadrants (Scale Up/Champions/Underperforming/Overspend) split by filtered-set medians; MIN_CAMPAIGNS=5 guard — shows "Not enough campaigns" message instead of chart when below threshold; "Based on selected channels" subtitle shown when isFiltered && hasEnoughCampaigns; safe log transform (logRoi: Math.log1p(Math.max(roi, -0.999))); top-2 campaign labels per quadrant by revenue; median reference lines in pink (#ec4899) on both axes; plugin id roiBudgetQuadrantPlugin (namespaced to avoid global collision with RoiCpaScatter)
│   │   │       ├── EmptyState.vue      # No-data screen — uses FileActions for download/upload buttons
│   │   │       ├── KpiCard.vue         # Single KPI metric card — props: label, value (string|null|undefined — pre-formatted by parent, falls back to 'N/A'); optional #secondary slot; kpi-label → kpi-value → kpi-secondary stack; uses @include cq-container('kpi-card') + @include cq-up(tiny, 'kpi-card') for container-query-driven font size scaling; flat scoped styles (no BEM)
│   │   │       ├── KpiBenchmarkDelta.vue # Directional delta indicator — props: current/benchmark (number|null), unit ('pp'|'pct'), lowerIsBetter?, showAbsoluteCurrency?; pp = percentage-point delta for rates (CTR), pct = relative % delta for costs (CPA); computes rawDelta, colorClass, arrow (↑/↓), formattedDelta, formattedAbsolute, formattedBenchmark; sign injected after € symbol for absolute currency delta; renders "[arrow] [delta]" colored + "[abs]" muted + "vs portfolio ([benchmark])" muted; scoped delta-positive/delta-negative/delta-neutral/delta-muted styles
│   │   │       ├── CampaignTable.vue   # Sortable campaign data table — prop: CampaignPerformance[]; reads pre-calculated roi/ctr/cvr/cpa directly; sort via useSort<SortField>("revenue","desc") aliased as sortField; handleSort wrapper casts string→SortField; channel cell uses `.badge.info.dimmed`
...
│   │   │       └── channel-filters/    # ChannelFilters module
│   │   │           ├── index.ts            # Barrel — exports ChannelFilters only
│   │   │           ├── ChannelFilterChips.vue  # Internal chip container — props: variant? ('visible'|'probe'), layout? ('strip'|'plain'), channels, totalCampaigns, selectedIds?, showAll?, allActive?, allReadonly?, singleRow?; renders "All" Chip + one Chip per channel (data-channel-id attr); probe variant is absolutely-positioned invisible measurement layer (aria-hidden, tabindex=-1, pointer-events-none) used by ChannelFilters to detect overflow without affecting layout; plain layout used inside dropdown (no flex-1/overflow-hidden, gap-2 p-3); single-row clamps height to max-h-9; exposes getRootEl(), getChannelChipEls(), hasOverflow() for parent measurement; emits clear / toggle (suppressed in probe mode); scoped SCSS with max-height via --channel-filter-max-height CSS var (default 4.8rem)
│   │   │           ├── ChannelFilters.vue  # Two-state filter strip — delegates chip rendering to ChannelFilterChips; measureRef (probe variant) drives hasOverflow via exposed hasOverflow(); chipsRef (visible variant) drives hiddenSelectedIds via exposed getChannelChipEls() + offsetTop; State A (no overflow): All chip + all channel chips interactive; State B (overflow): ChannelFiltersDialog trigger + single-row chipsRef showing only selected chips sorted by name; ResizeObserver on measureRef root; measure split into measureOverflow() + measureHidden() (nextTick-deferred); scoped SCSS: .channel-filters (relative flex items-start gap-2)
│   │   │           └── ChannelFiltersDialog.vue  # Consolidated overflow dialog — owns dropdownOpen state, trigger Button ref, FunnelIcon trigger button (info-outline paddingless), hidden-count Badge (small bold info, hidden while open), Dropdown, DropdownPanel, sticky dropdown header ("Channels" label + "Select all" ghost Button when hasSelection), scrollable ChannelFilterChips (layout="plain") content area; props: channels, selectedIds, hiddenCount; emits toggle/clear; props-only, no store reads; scoped SCSS: .filter-trigger-button (h-8 w-8, icon-wrapper centering, active state → bg-background border-info/65 text-info-light), .selected-filters-badge (absolute -top-2.5 -right-2.5), .dropdown-header (sticky, flex, border-b, h-11), .dropdown-title, .dropdown-content (max-h-[300px] overflow-y-auto)
...
│   │       │       ├── DataErrorsTable.vue # Dumb table component — props: errors (CampainDataRowError[]); single-column sort via useSort<string>("row") (dir only, key tracking irrelevant); no BEM
│   │       │       └── campain-duplications/
│   │       │           ├── DuplicationGroupHeader.vue # Group header content — props: campaignName, rowCount, isSelected, needsAttentionMode; emits clear; renders group-title-row span with campaign name/count, v-if/v-else-if/v-else Badge (success "Resolved" / danger "Needs Attention" / warning "Pending"), and destructive small Button "Clear selection" when isSelected; owns .group-title-row + .group-title-text scoped styles
│   │       │           └── CampainDuplicationsTable.vue # Sortable grouped duplicate table — props: duplicateGroups (CampainDataDuplicateGroup[]), requiredSelection?: boolean (default true); sort via useSort<SortKey>("rowId"); selection Map<campaignName, rowId>; needsAttentionMode computed (requiredSelection===true && selections.size===0); uses DuplicationGroupHeader per group header row; emits change:[Campaign[]] on every selection; 8-column table
│   │       │   └── ResolveDuplicationsStep.vue # Multi-root (body + ModalFooter) — uses DuplicateSummary (variant="resolve") + CampainDuplicationsTable; resolve-indicator shows resolvedCount/total (green when allResolved); canProceed: validCampaigns.length > 0 OR selectedCampaigns.length > 0; emits proceed([Campaign[]]) with campaigns from @change; Back/Proceed/Cancel buttons
│   │       ├── composables/
