...
│   │   │   ├── types/
│   │   │   │   └── index.ts            # AiProviderType, AiErrorCode (unified 10-code union: invalid-key/network/timeout/rate-limit/server-error/no-models/token-limit/invalid-response/parse-error/unknown), AiConnectionError (code: AiErrorCode), AiAnalysisError (code: AiErrorCode, message: string), AiAnalysisTab, ConfidenceLevel + shared building blocks (PerformanceDeltas, CampainSummaryTotals, AllocationShare, FunnelMetrics, PortfolioCount, Correlation) + prompt types (PromptList, PromptInstructions, PromptInstructionStep, PromptScopeConfig, BusinessContext, BudgetOptimizerContextInput) + data types (ExecutiveSummaryData incl. ExecutiveSummaryChannel/ExecutiveSummaryCampaign/ExecutiveSummaryOtherChannelsSummary, BudgetOptimizerData) + response types (BudgetOptimizerResponse, ExecutiveSummaryResponse with additional_channels_note? and stricter icon union); AiModel/ModelsResponse moved to providers/types.ts; AiAnalysisStatus/AiConnectionErrorCode/AiAnalysisErrorCode removed (replaced by AsyncStatus + AiErrorCode)
│   │   │   ├── prompts/
