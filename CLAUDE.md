...
│   │   │           └── ChannelFiltersDropdown.vue  # Floating dropdown panel — no self-positioning (parent Teleport wrapper owns fixed coords); all channels as toggle chips + sticky "Channels" header + "Clear all" (shown when selection exists); scrollable (max-h-[300px]); emits toggle/clear/close; props-only, no store reads
