...
│   │   ├── analysis/
│   │   │   ├── budget-optimization-analysis.types.ts # Legacy — ConfidenceLevel/ExecutionRisk; InefficientCampaignSignal, BudgetScalingCandidate, TransferCandidate, BudgetOptimizerAnalysis; kept until prompt integration migrates to PortfolioAnalysis
│   │   │   ├── budget-optimization-analysis.ts # Legacy — computeBudgetOptimizerAnalysis; kept until prompt integration migrates to PortfolioAnalysis
│   │   │   ├── executive-summary-analysis.types.ts # Legacy — SummaryAnalysis + signal types; kept until prompt integration migrates to PortfolioAnalysis
│   │   │   └── executive-summary-analysis.ts # Legacy — computeSummaryAnalysis; kept until prompt integration migrates to PortfolioAnalysis
│   │   ├── portfolio-analysis/
...
│   │   │   └── portfolio-analysis.ts # computePortfolioAnalysis(campaigns, channels, kpis, scope, filteredChannels) → PortfolioAnalysis — single assembly function; calls classifyCampaigns + classifyChannels + all signal helpers
│   │   └── data/
│   │       └── MOCK_CAMPAIN_DATA.ts # 21 mock campaigns across 13 real-world channels; exported as MOCK_CAMPAINS
│   ├── stores/
│   │   ├── campaignStore.ts    # Pinia store — title, portfolioChannels (Map<string, Channel> — single source of truth, set on init and in loadCampaigns), selectedChannelsIds (string[] of channel IDs); campaigns computed (flatMaps all ch.campaigns from portfolioChannels); selectedChannels computed (Channel[] for selected IDs or all); filteredCampaigns computed (flatMaps ch.campaigns from selectedChannels); kpis computed from selectedChannels (aggregates pre-summed Channel metrics); portfolioScope computed (campaigns/selectedCampaigns/selectedChannels string arrays); portfolioAnalysis computed (PortfolioAnalysis — full derived analysis from filteredCampaigns + selectedChannels + kpis + scope + filteredChannels flag); toggleChannel/clearFilters/loadCampaigns actions
