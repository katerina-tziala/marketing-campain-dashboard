...
│   │   │   ├── types/
│   │   │   │   └── index.ts            # AiProviderType, AiErrorCode (unified 10-code union), AiConnectionError, AiAnalysisError, AiAnalysisTab, ConfidenceLevel + shared building blocks (CampainSummaryTotals, AllocationShare, FunnelMetrics, PortfolioCount, Correlation) + prompt types (PromptList, PromptInstructions, PromptInstructionStep, PromptScopeConfig, BusinessContext, BudgetOptimizerContextInput) + data type (BudgetOptimizerData) + response types (BudgetOptimizerResponse, ExecutiveSummaryResponse — camelCase: healthScore/bottomLine/insights with metricHighlight/priorityActions/correlations; no icon, no key_metrics, no channel_summary)
│   │   │   ├── prompts/
...
│   │   │   │   ├── executive-summary-prompt2.ts # generateExecutiveSummaryPrompt(input: ExecutiveSummaryInput, filteredChannels: boolean) — assembles executive-summary AI prompt; camelCase schema; derivedSignals-first prioritization; strict noise-control and correlation rules
