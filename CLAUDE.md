...
│   │   ├── composables/
│   │   │   ├── useSort.ts          # useSort<T extends string>(defaultKey: T, defaultDir?: SortDir) → { sortKey, sortDir, toggleSort } — generic sort state composable; toggleSort flips dir on same key, resets to defaultDir on new key; used by CampaignDuplicationsTable, CampaignTable, GroupedCampaignTable, DataErrorsTable
...
│   │   │   │   ├── chart.types.ts    # BarChartData/Options/TooltipCallbacks/TooltipItem; DonutChartData/Options/TooltipCallbacks/TooltipItem/LegendLabelFilter; BubbleChartData/Options/TooltipCallbacks/TooltipItem/Plugin; ChartTickFormatter; ChartLegendPosition; ChartLegendItem ({ id, name, color })
...
│   │   │   ├── TableHeader.vue # Sortable thead — columns: DataTableColumn[]; position? (TableHeaderPosition); sortKey?; sortDir?; visuallyHiddenLabel?; emits sort; info palette for active sort; exports DataTableColumn + SortDir types
...
│   │   │   │   │   ├── AnalysisHeader.vue      # Tab header — props: title, actionLabel, isButtonDisabled, context (AnalysisPortfolioContext); emits: analyze; renders portfolio, channel, campaign metadata + portfolio period/industry (responsive: visible in modal layout only); Section + MetaRow (bullet)
...
│   │   │   │   │   ├── AiConnectionForm.vue        # Provider selection + API key + Connect button + Disclosure (instructions inline); props: resetKey? (number) used to reset form when panel closes
...
│   │   │   │   ├── gemini/             # index.ts, types.ts, api.ts (requestGeminiChatCompletion), connect.ts (connectGemini); extract-candidates/ subfolder (allowed-candidates.ts, sort-candidates.ts, index.ts — extractCandidates pipeline with stripPrefix on models/ prefix)
│   │   │   │   ├── qroq/               # index.ts, types.ts, api.ts (requestGroqChatCompletion), connect.ts (connectGroq); extract-candidates/ subfolder (allowed-candidates.ts, sort-candidates.ts, index.ts) (folder name: qroq)
│   │   │   │   └── utils/              # error-handling.ts, models-utils.ts (getAllModelsLimitReached, getModelById, getNextAvailableMode), providers-meta.ts (ProviderHelpStep with optional linkText/href; PROVIDER_LABELS, PROVIDER_HELP with Groq Console + Google AI Studio links, PROVIDER_OPTIONS), stability-penalty-by-model-id.ts (stabilityPenaltyByModelId — penalizes preview/experimental/latest/beta IDs), shared.ts (parseJsonResponse); index.ts barrel re-exports all modules
│   │   │   ├── types/
...
│   │   │   ├── components/
│   │   │   │   ├── index.ts            # Barrel — exports CampaignPerformanceHeader, ChannelFilters, Kpis (CampaignTable + GroupedCampaignTable consumed via direct imports from CampaignPerformanceView)
...
│   │   │   │   ├── GroupedCampaignTable.vue # Sortable campaign table grouped by channel — props: campaigns (CampaignPerformance[]); aggregates per-channel totals via aggregateCampaignMetrics + computePerformanceMetrics; expand/collapse all + per-row toggle (Vue Transition with grid-template-rows animation); sort via useSort / sortByValue(); uses TableGroupHeaderRow + ChevronIcon; PerformanceIndicator on Revenue/CVR/ROI
...
│   │       ├── RevenueVsBudgetChart.vue # Toggle wrapper — props: channels; owns RadioToggle (Performance/Efficiency) state and switches between RevenueVsBudgetBars + EfficiencyGapBars sub-charts
│   │       ├── RoiVsBudgetScaling.vue # ROI vs Budget scaling card — owns card shell, title, "Based on selected channels" subtitle, median summary (MetaRow), limited-data info state (Notification when < MIN_CAMPAIGNS); passes campaigns, medians, highlightCampaignsByQuadrant (RoiBudgetScalingHighlights) into RoiVsBudgetScatterChart; default chart height from !h-29
...
│   │       │   └── roi-budget-scaling-chart.config.ts   # ROI scaling chart config — ROI_BUDGET_SCALING_QUADRANTS (Scale Up/Champions/Monitor/Overspend), ROI_BUDGET_SCALING_POINT_RADIUS, ROI_BUDGET_SCALING_HIGHLIGHTED_POINT_RADIUS, ROI_BUDGET_SCALING_MIN_CAMPAIGNS (=5), ROI_BUDGET_SCALING_BUDGET_AXIS_ROUNDING, ROI_BUDGET_SCALING_TICK_VALUES, ROI_BUDGET_SCALING_DIVIDER_STYLE, QUADRANT_BACKGROUNDS
│   │       ├── types/
│   │       │   ├── index.ts
...
│   │       │   └── roi-budget-scaling-chart.types.ts # RoiBudgetScalingHighlights (scaleUp/champions/monitor/overspend: string[]), RoiBudgetScalingMedians, RoiBudgetScalingQuadrantConfig, RoiBudgetScalingQuadrantKey
│   │       └── utils/
│   │           ├── index.ts
...
- At the start of every session, read `.claude/memory/MEMORY.md` to load the memory index, then read any files relevant to the current task.
