...
│   │   └── index.ts            # Barrel export for the full ui library — re-exports charts/*, icons/*, toast/*, types/*, meta/*; exports Tabs, BaseModal, Spinner, DataTableHeader, FileDropzone, PasswordInput, RadioToggle, Button, ButtonVariant, ButtonSize, Card, CardVariant, Badge, SectionHeaderLayout directly
│   ├── shell/
...
│   │   │   │           ├── ExecutiveSummaryAnalysis.vue        # Executive Summary tab — thin orchestrator; receives scope: PortfolioScope prop; uses SectionHeaderLayout with #header slot (dynamic title: "Portfolio Summary" / "Performance Summary") and #action slot (Button.primary.square + MagicWandIcon); MetaRow with portfolio title/channel count/campaign count below header; wraps AnalysisState (no panel-head — button lives in action slot above); slot content guarded with v-if="response" (null-safe); reads aiAnalysis.store + campaign.store; scoped .header-title
