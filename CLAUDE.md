...
│   │   ├── types/
│   │   │   └── campaign.ts     # Campaign interface + CampaignKPIs interface (incl. totalConversions) + CampaignScope interface (campaigns/selectedCampaigns/selectedChannels string arrays)
│   │   ├── utils/
...
│   ├── stores/
│   │   ├── campaignStore.ts    # Pinia store — campaigns, title, filters, KPIs (incl. totalConversions); campaignScope computed (CampaignScope — campaigns/selectedCampaigns/selectedChannels as string arrays); loadCampaigns action
