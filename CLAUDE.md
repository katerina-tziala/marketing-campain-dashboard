...
│   │   │   ├── ai-analysis/
│   │   │   │   ├── utils/
│   │   │   │   │   ├── analysis-badge-variants.ts  # Badge variant helpers for AI panels — internal badgeVariant(map, key) generic resolver; exports: channelStatusVariant, confidenceVariant, executionRiskVariant, actionVariant, effortVariant (urgencyVariant + insightTypeVariant moved into their respective components)
...
│   │   │   │       │   └── BudgetRecommendations.vue       # Recommendations — props: recommendations[]; fromCampaign→toCampaign header with arrow; confidenceVariant + executionRiskVariant badges; budgetShift/expectedImpact metrics formatted with formatCurrency/formatPercentage; v-if on length; rec-card container-type for badge stacking via @container
