...
│   ├── stores/
│   │   ├── campaignStore.ts    # Pinia store — campaigns (CampaignPerformance[], mapped via toCampaignPerformance on load), title, filters, KPIs (incl. totalImpressions, totalClicks, totalConversions); campainChannels ref (Map<string, Channel> via buildChannelMap — set on init and in loadCampaigns); channelTotals computed (Record<string, ChannelTotals> via groupByChannel); campaignScope computed (CampaignScope — campaigns/selectedCampaigns/selectedChannels as string arrays); loadCampaigns action
