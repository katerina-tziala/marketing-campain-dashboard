...
│   │   │   ├── ClockIcon.vue       # Clock face with hour/minute hand — used in Pending badge in CampainDuplicationsTable
│   │   │   ├── CloseIcon.vue
│   │   │   ├── DownloadIcon.vue
...
│   │       │       └── CampainDuplicationsTable.vue # Sortable grouped duplicate table — props: duplicateGroups (CampainDataDuplicateGroup[]), requiredSelection?: boolean (default true); sort via useSort<SortKey>("rowId"); selection Map<campaignName, rowId>; group header: v-if/v-else-if/v-else Badge per state (success+CheckIcon "Resolved" / danger+AlertTriangleIcon "Needs Attention" / warning+ClockIcon "Pending"); needsAttentionMode computed (requiredSelection===true && selections.size===0); destructive small Button "Clear selection" when group selected; emits change:[Campaign[]] on every selection; 8-column table
