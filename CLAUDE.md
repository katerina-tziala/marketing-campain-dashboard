...
│   │   └── aiAnalysis.store.ts  # Pinia store — shared AI analysis logic for both tabs; keyed by AiAnalysisType ('budgetOptimizer'|'executiveSummary'); per-tab internal state (plain object): firstAnalyzeCompleted, controller, debounceTimer, cache (Map<string, CacheEntry> — response/timestamp/cooldownUntil), lastVisibleCacheKey; no dataCache — Vue computed on campaign.store portfolioAnalysis handles memoization; per-tab reactive display state (ref<TabDisplay<T>>): budgetOptimizer { status, response: BudgetOptimizerResponse|null, error: AiAnalysisError|null, notice: AiAnalysisNotice|null } + executiveSummary { same shape, ExecutiveSummaryResponse|null }; shared: activeTab (AiAnalysisType), analysisActivated; derived getters from useAiConnectionStore: tokenLimitReached (computed from allModelsLimitReached), evaluationDisabled (evaluationDisabled || filteredCampaigns.length === 0); store-internal helpers: isBelowOptimizerMinimum() (filteredCampaigns.length < 2 — used by optimizerCanAnalyze as pure check), showOptimizerMinimumError(tab) → bool (sets error display + returns true when below minimum — used for early-return in executeAnalysis/evaluateTab/watcher), showCachedResult(tab, cacheKey) → bool (looks up cache, calls setDisplay + updates lastVisibleCacheKey, returns whether hit), showTokenLimitState(tab) (delegates to showCachedResult or sets token-limit error), revertTab(tab) (calls cancelActiveRequest then shows last known cache entry or idle); module-level helpers: getOtherAnalysisType, setDisplay (full ref.value replacement — no property mutation), createTabState, TabDisplay<T> type; store saves AiAnalysisError { code, rawMessage? } — no message lookup at store level; message text resolved by AnalysisState.vue from analysis-messages.ts; notice (AiAnalysisNotice { code: AiAnalysisNoticeCode }) replaces errorFallback — used for stale-result state alongside cached response; debounced channel-filter auto-calls (350ms), response caching (provider::sorted labels), request cancellation (AbortController), 5s cooldown, silent model fallback on token-limit, stamps response.model + response.timestamp on success, model change watcher, setActiveTab uses revertTab on previous tab, onPanelClose uses revertTab on loading tabs, panel open/close + tab switch evaluation, clearStateForNewCSV, clearStateForDisconnect
│   ├── router/
...
│   ├── shell/
│   │   ├── AppShell.vue            # Top-level layout wrapper — flat @apply styles (shell-left/shell-header/shell-title/shell-main); flex col → flex row at lg+; shell-left (header + shell-main slot, flex col, overflow-y auto) + AiToolsDrawer sibling; shell-main has max-width 1280px centered; provides openUploadModal and openAiPanel via provide(); uses useAiConnectionStore.aiPanelOpen for panel state; wires panel open/close to aiAnalysis.store; header "Upload CSV" button uses .btn-secondary-outline and routes through ReplaceDataModal when data exists; gradient title (indigo→pink)
...
│   │   │   │   │   ├── utils.ts            # getCacheKey(channelIds, provider) → string — normalises channelIds (sort, fallback to 'all') and lowercases provider; used by aiAnalysis.store
│   │   │   │   ├── types/
...
│   │   │   │       │   ├── BudgetOptimizationAnalysis.vue        # Budget Optimizer tab — thin orchestrator; receives scope: PortfolioScope prop; wraps AnalysisState; slot content guarded with v-if="response" (null-safe); reads aiAnalysis.store only; no scoped styles
...
│   │   │   │           ├── ExecutiveSummaryAnalysis.vue        # Executive Summary tab — thin orchestrator; receives scope: PortfolioScope prop; wraps AnalysisState; slot content guarded with v-if="response" (null-safe); reads aiAnalysis.store only; no scoped styles
...
│   │       │   └── useUploadModal.ts       # Upload modal composable — accepts modalRef (InstanceType<UploadModal>); uses campaign.store internally; hasCampaigns computed; requestUpload (opens modal or shows replace confirm based on hasCampaigns); onReplaceConfirm/closeReplaceConfirm; calls provide('openUploadModal') internally
│   │       └── utils/
