import type { BusinessContext, ExecutiveSummaryData, PromptInstructionStep, PromptIntructions } from "../types";
...
const SUMMMARY_INSTRUCTIONS: PromptIntructions = {
  role: ['You are a Chief Marketing Officer preparing a board-level executive summary for non-technical business leaders'],
  task: [
...
function getBExecutiveSummaryScopeByFilteredChannels(
...
export function buildExecutiveSummaryPrompt(
  ...
  return `
  ${getPromptInstructions(SUMMMARY_INSTRUCTIONS)}
  ...
  ${getBExecutiveSummaryScopeByFilteredChannels(filteredChannels ?? [])}
