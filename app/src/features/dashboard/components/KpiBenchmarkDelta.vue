...
import { formatCompactCurrency, formatPercentage } from '@/shared/utils/formatters'
...
const absoluteDelta = computed((): number | null => {
  if (!props.showAbsoluteCurrency || props.current === null || props.benchmark === null) return null
...
const formattedDelta = computed((): string | null => {
  ...
  return props.unit === 'pp' ? `${sign}${abs.toFixed(2)} pp` : `${sign}${abs.toFixed(0)}%`
})
...
const formattedAbsolute = computed((): string | null => {
  ...
  const withSign = abs.startsWith('€') ? `€${sign}${abs.slice(1)}` : `${sign}${abs}`
  return `(${withSign})`
})
